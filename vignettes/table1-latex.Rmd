...
The output is less attractive in my opinion, and less consisten with the
typical look of LaTeX tables and documents. It is more spaced out, doesn't have
bold labels or line break in the column headers (actually, this is a problem
common to both packages, but the `t1kable()` and `t1flex()` functions deal with
it differently because `flextable` will actually include the line breaks in
HTML and .docx output, and the overall best approach is unclear), and doesn't
use the `booktabs` package (the thickness of the horizontal rules is
...
`flextable` in a LaTeX documents.  But a big advantage of `flextable` is that
in can render to .docx format (i.e.  Microsoft Word), which `kableExtra`
cannot.
