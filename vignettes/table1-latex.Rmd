...
Note that this issues a warning about using the `xelatex` engine instead of
`pdflatex`, but we will ignore that here because vignettes don't build with
`xelatex`.

The output is less attractive in my opinion, but could still be considered
acceptable. It is more spaced out, doesn't have bold labels or line break in
the column headers.  Also, `flextable` places the table in a float, whereas
