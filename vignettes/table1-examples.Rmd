...
variables to include in the table are separated by '+' symbols, the stratification
variable (which creates the columns) appears to the right of a "conditioning"
symbol '|', and the data frame containing the variables in the formula is given
as the `data` argument.

By without giving the variables and categories nice labels, the result doesn't
look great. To improve it, we can do the following:
...
That's better, but still not quite the same as the original blog post. First,
in the blog post the "Total" column in the first on the left, while we have it
on the right. Second, the two "Death" strata (Melanoma and Non-melanoma) are
grouped together under a common heading. Finally, the continuous variables Age
and Thickness show only Means (SD), not Median [Min, Max] like the `table1`
default output (I will not concern myself with the footnote here, but it can
easily be added). To achieve the same result, we need to customize the output
...
table1(strata, labels, groupspan=c(1, 1, 2),
       render.continuous=myrendercont, render.categorical=myrendercat)
```
...
groups, and we want to compare those groups with respect to the distribution of
baseline characteristics, and for this purpose having percentages add up to
100% within columns makes the most sense. Let's continue with an example of
that nature, using simulated data.
...
dat$treat <- f(c("Placebo", "Treated"), n, prob=c(1, 2))
dat$age   <- runif(n, 18, 65)
dat$sex   <- f(c("Female", "Male"), n, prob=c(.6, .4))
...
Note that when a continuous variable contains missing values (here weight),
these are reported as a distinct category.
...
Finally, we may again consider something more complicated, using the default
(i.e., non-formula) interface. Suppose that instead of simply being assigned to
placebo or active treatment, there were actually two doses of treatment
randomized, 5 mg and 10 mg.
...
Markdown](rmarkdown.rstudio.com/), along with the `results='asis'` chunk option
(at least, that's how I use it). You embed your own CSS in the document by
including this in the YAML header of your file (where style.css is the name of
the file that contains the CSS):
